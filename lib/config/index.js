...
const configFilePath = path.resolve(
  appRootPath,
  process.env.CMD_CONFIG_FILE || "config.json"
);
const fileConfig = fs.existsSync(configFilePath)
  ? require(configFilePath)[env]
  : undefined;
...
if (["debug", "verbose", "info", "warn", "error"].includes(config.loglevel)) {
  ...
  logger.error(
    "Selected loglevel %s doesn't exist, using default level 'debug'. Available options: debug, verbose, info, warn, error",
    config.loglevel
  );
}
...
if (config.ldap.tlsca) {
  ...
  config.ldap.tlsOptions = config.ldap.tlsOptions
    ? Object.assign(config.ldap.tlsOptions, tlsOptions)
    : tlsOptions;
}
...
config.isBitbucketEnable =
  config.bitbucket.clientID && config.bitbucket.clientSecret;
config.isGitLabEnable = config.gitlab.clientID && config.gitlab.clientSecret;
config.isMattermostEnable =
  config.mattermost.clientID && config.mattermost.clientSecret;
config.isLDAPEnable = config.ldap.url;
config.isSAMLEnable = config.saml.idpSsoUrl;
config.isOAuth2Enable = config.oauth2.clientID && config.oauth2.clientSecret;
config.isPDFExportEnable = config.allowPDFExport;
config.isEthereumEnable = config.ethereum && config.ethereum.enabled;

// Check gitlab api version
if (
  config.gitlab &&
  config.gitlab.version !== "v4" &&
  config.gitlab.version !== "v3"
) {
  logger.warn(
    "config.js contains wrong version (" +
      config.gitlab.version +
      ") for gitlab api; it should be 'v3' or 'v4'. Defaulting to v4"
  );
  config.gitlab.version = "v4";
}
// If gitlab scope is api, enable snippets Export/import
config.isGitlabSnippetsEnable =
  (!config.gitlab.scope || config.gitlab.scope === "api") &&
  config.isGitLabEnable;
...
for (let i = keys.length; i--; ) {
  ...
  if (
    uppercase.test(keys[i]) &&
    config[lowercaseKey] !== undefined &&
    fileConfig[keys[i]] === undefined
  ) {
    logger.warn(
      "config.js contains deprecated lowercase setting for " +
        keys[i] +
        ". Please change your config.js file to replace " +
        lowercaseKey +
        " with " +
        keys[i]
    );
...
if (Object.keys(process.env).toString().indexOf("HMD_") !== -1) {
  logger.warn(
    "Using legacy HMD prefix for environment variables. Please change your variables in future. For details see: https://hackmd.io/c/codimd-documentation/%2F%40codimd%2Fmigrate-2-0#1-Drop-old-environment-variables-support"
  );
}
...
if (config.sessionSecret === "secret") {
  ...
  config.sessionSecret = crypto
    .randomBytes(Math.ceil(config.sessionSecretLen / 2)) // generate crypto graphic random number
...
if (
  ["filesystem", "s3", "minio", "imgur", "azure", "lutim"].indexOf(
    config.imageUploadType
  ) === -1
) {
  logger.error(
    '"imageuploadtype" is not correctly set. Please use "filesystem", "s3", "minio", "azure", "lutim" or "imgur". Defaulting to "filesystem"'
  );
